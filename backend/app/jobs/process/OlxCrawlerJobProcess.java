...
import olx.rules.OlxCrawlerOnline;
...
public class OlxCrawlerJobProcess {
	
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public static List<Anuncio> getAnuncios(String estado, String marca, String modelo) throws Exception {
		 ...
		 */
		controller.addSeed(escapeHtml(CrawlerConstants.CRAWLER_PATTERN_OLX.replace("{estado}", estado).replace("{marca}", marca)
		 ...
		 */
		controller.start(OlxCrawlerOnline.class, numberOfCrawlers);
			...
			for (Object object : (List) anuncios) {
				if (null != object && object instanceof ArrayList) {
					for (Object object2 : (List) object) {
						if (null != object2 && object2 instanceof Anuncio) {
							anunciosReais.add((Anuncio) object2);
						}
					}
				} else if (null != object && object instanceof Anuncio) {
